...
export function ArcadeMachine({ visualState, teams, selectedTeam, onPush, onUndo, soundEnabled, disabled, historyDisabled, drawButtonLabel = 'BỐC THĂM', progress }: ArcadeMachineProps) {
  ...
  return <section className={`arcade-machine arcade-machine--${visualState} pixel-frame`} data-arcade-machine aria-label="Khu vực bốc thăm">
    <header className="arcade-machine-heading">
      ...
      <div className="arcade-pool-count"><strong>{teams.length} ĐỘI</strong><span>{progress ?? `${teams.reduce((total, team) => total + team.participants.length, 0)} VĐV`}</span></div>
    ...
    <div className="arcade-control-deck pixel-frame">
      ...
      <div className="arcade-console-mark pixel-frame" role="status"><span aria-hidden="true">»</span><div><b>{statusByState[visualState]}</b><small>{disabled ? (visualState === 'complete' ? 'CHÚC CÁC ĐỘI THI ĐẤU THẬT TỐT!' : 'MAY MẮN SẼ GỌI TÊN AI?') : 'NHẤN SPACE ĐỂ BẮT ĐẦU'}</small></div><span aria-hidden="true">«</span></div>
        <div className="arcade-push-housing"><button className="arcade-push pixel-frame" onClick={push} disabled={disabled} aria-label={drawButtonLabel}><span aria-hidden="true">»</span><b>{drawButtonLabel}</b><PixelBall /></button></div>
    </div>
  </section>
}
